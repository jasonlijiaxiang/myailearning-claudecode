...
**「产出 skill 版本」字段的口径（2026-07-22 澄清）**：记**首版产出**时的技能版本，
后续增补不覆盖它。用途是"技能大改后，哪些模块还是老规范产出的"——覆盖成"最近一次触碰"
这个用途就失真了。每次执行用的版本记在「最后更新」的散文里（那里本来就按时间排流水）。
取证：网页版八批扩建后，各册这个字段仍是 v1.0/v2.0/v3.0 不等，如果当时改成"最近触碰"，
19 册会齐刷刷变成同一个版本号，"谁还没按新规范重做过"就再也查不出来了。
...
1. **能追出真实日期就用真实的**——这是最好的情况。追法：题目出自讲义哪一章，就查该模块
   ...
   **网页版批次的定版取值**（八批实践收敛）：讲义衍生题 = **该章账本的核实/增章日期**
   （同一册里不同章的题因此会带不同日期，正好如实反映哪些是后来增章带进来的）；
   网页版新立的题 = **当日**。页面顶部再给一行基线声明，说明这两个日期各代表什么。
