...
**存量用一行基线声明覆盖，不逐条补**（2026-07-20 定版）：在问答区顶部写一句
「以下题目为 YYYY-MM-DD 基线；此后新增的题各自标注加入日期」。

为什么不给存量逐条盖上"今天"：那些题是更早写的，**盖今天等于写下一个假日期**，与
「不断言未经核实的事实」冲突；而基线声明拿到的功能完全一样——**此后带日期的就是新加的**——
既不假、也不用给几十条塞标签。回头考据每条的真实加入日则成本高、易考错，
**不确定的日期比没有日期更坏**。
