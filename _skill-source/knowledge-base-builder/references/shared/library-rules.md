...
更好看，也更适合当"门户页"长期维护。保持轻量：内联 `<style>`，不依赖外部 CSS/JS，能本地直接打开。

### 门户页色板（README/_prep 等 HTML 成品，全库统一）

HTML 门户面与讲义 PPT 是**两套并行的识别色**，各自统一、不得引入第三种（2026-07-14 审查
把实库 20 个 HTML 页面已收敛的用色固化为规则）：

| 角色 | 令牌 |
| --- | --- |
| 深底/标题深块 | `#0B2540`（与讲义共用的深海军蓝） |
| 门户强调/链接/层卡 | `#2B6CB0`（门户蓝——只用于 HTML，不进讲义） |
| 扩展位/虚线卡 | `#D9A62D` 金 + `#FDF8E8` 浅金底 |
| 完成/通过 | `#2F855A` 绿 |
| 正文/次级 | `#1A2733` / `#51606E` |
| 浅底 | `#FFFFFF` 主底、`#F7FAFC` 斑马纹/浅卡 |

新建模块 README、改顶层索引、做 _prep 类取用页时按此表取色；讲义色板（ppt-design-system）
里的主强调青 `#128199` 不要混进门户页，门户蓝也不要混进讲义。

### 根总纲 README.html 骨架（面中立）

样式两页共用一份内联 `<style>`（面总览照抄，别各写一套）。
...
同样内联样式，列出本模块章节表 + 每节状态（用 `.ok`/`.todo` 上色），并回链**根总纲**
