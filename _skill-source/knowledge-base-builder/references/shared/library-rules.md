...
## 章节清单
| 章节 ID | 现编号 | 标题 | 状态 | 最后核实 | 新增 |
| --- | --- | --- | --- | --- | --- |
| rag-what-why | 第 1 章 | 是什么/为什么 | ✅ | 2026-07-07 | — |
| rag-graphrag | 第 10 章 | GraphRAG | ✅ | 2026-07-20 | 2026-07-20 |
...
## 新增标注：大块内容记下它是哪天加进来的（2026-07-20 起，只对今后生效）

知识库是长期资产、持续生长。读者（和半年后的你自己）翻到某一节时会想知道**这是本来就有的，
还是最近才加的**——尤其是回头找"上次补的那块讲什么来着"。所以从今天起给新增内容标日期。

**什么算"大块"，需要标：**

- 新增一个**章**；
- 新增一个**独立小节**（讲义里一组完整的页、网页里一个 `h3` 级小节）；
- 新增一道**问答/对练题**。

**什么不标**：改措辞、更新数字、修错、配图、重排版式、改章节顺序——这些是对既有内容的
维护，不是新知识点。**判据一句话：读者会不会觉得"这块我上次没见过"**；不会就不标。

**记在哪儿：**

| 位置 | 怎么写 |
| --- | --- |
| MANIFEST 章节清单 | 末尾「新增」列填 ISO 日期；存量行填 `—` |
| 讲义页面 | 章扉或小节标题旁写「新增 YYYY-MM-DD」 |
| 网页页面 | 标题或问答条目旁给 `<span class="added">新增 YYYY-MM-DD</span>` |

**存量一律不补**（用户 2026-07-20 决定）：回头给 19 册几百个章节考据"当初哪天加的"，
成本高、价值低，还容易考据错——**不确定的日期比没有日期更坏**。存量就留 `—`。

**为什么这一条不违反「真源唯一、派生只引用」**：新增日期是**历史不变量**——写下那一刻就
永远不会变。派生账之所以危险，是因为真源会变而副本不跟着变（页数、章数、模块数都是这样）；
不变量多处登记不会打架。所以这里允许账本与成品各记一份，不必建回刷机制。

**顺带的好处**：MANIFEST 那一列是机器可读的，网页面可以据此生成"最近新增"视图——
跨模块按时间排开，正是 `web-rules.md` §六·六 说的"纸面做不到的事"。
